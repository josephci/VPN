...
**方法 4：USB 線 / USB 手指（零網絡記錄）**

手機（有代理）落載好個 zip → 插 USB 線 → 電腦直接複製。純物理傳輸，五秒搞掂。iPhone 麻煩啲（要經「檔案」app），但用 USB 手指 + 讀卡器都做得到。

**方法 5：微信／QQ 文件傳輸 —— 最後手段**

技術上多數傳得到（有時會俾檔案掃描攔住），**但呢個係留痕最多嘅方法**：
...
就算傳得過，你已經留低咗一條記錄：某實名帳號某時傳送咗一個翻牆客戶端。個人自用被追究嘅機會低，但**有更好嘅選擇就冇必要**。
...
| 🥈 | USB 線／手指 | **零網絡記錄** |
| 🥉 | 自己部伺服器 scp | 只有你同你部機 |
| 4 | Oracle Cloud Shell | Oracle 有 log，但唔受內地法規約束 |
| ❌ | 微信／QQ | 騰訊伺服器 + 實名帳號 + 完整記錄 |
